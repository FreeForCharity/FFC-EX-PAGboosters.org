...
you're gonna have to get over it :) If `standard` is good enough for `pnpm`, it's good enough for `fs-extra`.
...
- `pnpm run lint`: runs the linter ([standard](http://standardjs.com/))
- `pnpm run unit`: runs the unit tests
- `pnpm run unit-esm`: runs tests for `fs-extra/esm` exports
- `pnpm test`: runs the linter and all tests
